...
class Model(object):
    def __init__(self,is_training=True,layer=None,target=None,img_real=None):
        ...
        if(is_training):
            ...
            self.x=tf.Variable(self.img_real)
        else:
        ...
        if is_training:
            ...
            self.l2diff = tf.sqrt(tf.reduce_sum(tf.square(tf.subtract(self.fc8, target)),
                                           reduction_indices=1))

            self.regularizer =0.0001*tf.nn.l2_loss(self.x)

            self.loss=self.l2diff+self.regularizer
            self.opt=tf.train.AdamOptimizer(1).minimize(self.loss,var_list=[self.x])
            '''

            self.loss=-1*self.prob[0][target]
            self.opt = tf.train.AdamOptimizer(0.005).minimize(self.loss, var_list=[self.x])
